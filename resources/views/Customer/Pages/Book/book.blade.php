        ...
        <div class="container-fluid">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="small-box">
                                ...
                                <img class="img_fix_size_f"
                                    src="{{ isset($function_hall['function_hall_image']) ? asset('functional_hall_images/' . $function_hall['function_hall_image']) : asset('dist/img/default.png')}}">
                                <!-- </div> -->
                                <div class="fh_res">
                                    <div class="fh_desc">{{ $function_hall['function_hall_description'] ?? ''}}</div>
                                    <div class="fh_txt">RATE:
                                        P{{ number_format($function_hall['function_hall_rate'] ?? '', 2, '.', ',') }}
                                        per 5hours</div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12 d-flex justify-content-end">
                            <button class="btn btn-md card-header-color" id="function_hall_btn" data-toggle="modal"
                                data-target="#book_now_d"
                                onclick="edit({{ $function_hall['function_hall_rate'] ?? '' }}, {{ $function_hall['id'] ?? '' }}, 'hall')">
            ...
            <div class="divide_box">
                ...
                <div class="card m-1">
                    ...
                    <div class="card-body">
                        <div id="carouselExampleAutoplaying{{ $loop->index }}" class="carousel slide room-item"
                            data-bs-ride="carousel">
                            <div class="carousel-inner">
                                @foreach ($room['room_image'] as $index => $room_image)
                                <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                                    <img src="{{ asset('/images/' . $room_image) }}" class="img_fix_size"
                                        alt="Room Image">
                                </div>
                                @endforeach
                            </div>
                            ...
                            <button class="carousel-control-prev" type="button"
                                data-bs-target="#carouselExampleAutoplaying{{ $loop->index }}" data-bs-slide="prev">
                                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                <span class="visually-hidden">Previous</span>
                            </button>
                            <button class="carousel-control-next" type="button"
                                data-bs-target="#carouselExampleAutoplaying{{ $loop->index }}" data-bs-slide="next">
                        ...
                        <div class="row">
                            <div class="col-sm-12 d-flex justify-content-end">
                                <button class="btn btn-md card-header-color" id="room_btn" data-toggle="modal"
                                    data-target="#book_now_d"
                                    onclick="edit({{ $room['room_rate'] }}, {{ $room['id'] }}, 'room')">
                                    <i class="fas fa-tag"></i>
                                    Book Now!
                                </button>
                            </div>
                ...
                <form action="{{ route('book_store') }}" class="formPost">
                    <div class="row">
                        <div class="col">
                            ...
                            <input type="text" class="form-control mb-3" id="auth_id" name="auth_id"
                            ...
                            <input type="text" class="form-control mb-3" id="room_fullname" name="room_fullname"
                                readonly required placeholder="Full Name"
                                value="{{ $current_user['ordinary_user_fullname'] }}">
                            <input type="text" class="form-control mb-3" id="room_address" name="room_address" readonly
                                required placeholder="Address" value="{{ $current_user['address'] }}">
                            <input type="text" class="form-control mb-3" id="room_contact_number"
                                name="room_contact_number" readonly required placeholder="Contact Number"
                                value="{{ $current_user['c_number'] }}">
                            ...
                            <select class="form-control mb-3 text-center" name="payment_method" id="payment_method"
                                required>
                            ...
                            <input type="text" class="form-control mb-3" id="room_rate" name="room_rate" readonly
...
$('#payment_method').change(() => {
    if ($('#payment_method').val() < 1) {
        let div_payment =
            '<input type="number" class="form-control mb-3" id="payment" name="payment" placeholder="Payment" required>';
        $('#div_payment').html(div_payment);
    } else {
        let div_payment =
            '<div class="center_img"><input type="text" class="form-control "placeholder="09925312738" disabled><img src="' +
            '{{asset('/images/qrcode.jpg')}}' + '" class="rqcode_image mb-2"><img></div><input type="text" class="form-control mb-3" id="reference_num" name="reference_num" placeholder="Reference Number" required><input type="number" class="form-control mb-3" id="payment" name="payment" placeholder="Payment" required>';
        $('#div_payment').html(div_payment);
    }
});
